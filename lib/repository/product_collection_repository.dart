...
class ProductCollectionRepository extends PsRepository {
  ProductCollectionRepository(
      {@required PsApiService psApiService, @required ProductCollectionDao productCollectionDao}) {
  ...
  void sinkProductListStream(StreamController<PsResource<List<ProductCollectionHeader>>> productCollectionListStream,
  ...
  void sinkProductStream(StreamController<PsResource<ProductCollectionHeader>> productCollectionStream,
  ...
  Future<dynamic> insert(ProductCollectionHeader productCollectionHeader) async {
    return _productCollectionDao.insert(_primaryKey, productCollectionHeader);
  }

  Future<dynamic> update(ProductCollectionHeader productCollectionHeader) async {
    return _productCollectionDao.update(productCollectionHeader);
  }

  Future<dynamic> delete(ProductCollectionHeader productCollectionHeader) async {
    return _productCollectionDao.delete(productCollectionHeader);
  }

  Future<dynamic> getProductCollectionList(
      StreamController<PsResource<List<ProductCollectionHeader>>> productCollectionListStream,
    ...
    sinkProductListStream(
        productCollectionListStream, await _productCollectionDao.getAll(finder: finder, status: status));
    ...
    if (isConnectedToInternet) {
      ...
      if (_resource.status == PsStatus.SUCCESS) {
        ...
        sinkProductListStream(productCollectionListStream, await _productCollectionDao.getAll(finder: finder));
      }
    }
  }
  ...
  Future<dynamic> getNextPageProductCollectionList(
    ...
    if (isConnectedToInternet) {
      ...
      if (_resource.status == PsStatus.SUCCESS) {
        _productCollectionDao.insertAll(_primaryKey, _resource.data).then((dynamic data) async {
          sinkProductListStream(productCollectionListStream, await _productCollectionDao.getAll(finder: finder));
        });
      } else {
        sinkProductListStream(productCollectionListStream, await _productCollectionDao.getAll(finder: finder));
